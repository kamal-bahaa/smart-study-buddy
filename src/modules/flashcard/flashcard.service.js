...
const assertFlashcardOwnership = async (flashcardId, userId) => {
    const flashcard = await prisma.flashcard.findUnique({
        ...
        select: {
            id: true, front: true, back: true, difficulty: true, createdAt: true,
...
export const generateFlashcardsStream = async (pdfId, userId, res) => {
    ...
    try {
        ...
        res.write(`event: start\ndata: ${JSON.stringify({ totalChunks: chunks.length, fileName: document.fileName })}\n\n`);
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            try {
                ...
                if (response.data?.success && Array.isArray(response.data.flashcards)) {
                    const newFlashcards = response.data.flashcards.filter(({ question }) => {
        ...
        if (allFlashcards.length === 0) {
            res.write(`event: error\ndata: ${JSON.stringify({ message: 'AI service returned no flashcards' })}\n\n`);
            res.end();
            return;
        }
        ...
        await prisma.$transaction([
            ...
            prisma.flashcard.createMany({
                data: allFlashcards.map(({ question, answer, difficulty }) => ({
                    front: question, back: answer, difficulty: difficulty ?? 'easy', pdfId,
                })),
            }),
        ]);
...
export const generateFlashcards = async (pdfId, userId) => {
    ...
    try {
        ...
        if (allFlashcards.length === 0) throw ApiError.internal('AI service returned no flashcards');
